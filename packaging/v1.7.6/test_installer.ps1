...
$root=Join-Path $PSScriptRoot 'installer_test_v176_final'
$appDir=Join-Path $root 'App'
$dataDir=Join-Path $root 'Data'
New-Item -ItemType Directory -Path $appDir,$dataDir -Force | Out-Null
Set-Content -LiteralPath (Join-Path $appDir 'sentinel.txt') -Value 'pre-update application file'
$db=Join-Path $dataDir 'contractor_tracker.db'
Copy-Item -LiteralPath 'C:\Users\Kyle\Downloads\contractor_tracker_backup_20260923_111706_today.db' -Destination $db
...
$setup=Join-Path $PSScriptRoot 'release\ConTracktor_v1_Update_1.7.6.exe'
