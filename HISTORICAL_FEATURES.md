# Historical feature and deployment notes

This is the retained pre-v1.7.3 documentation for audit/history. Installation and security instructions below may refer to older releases. Use UPDATE_GUIDE_1.7.3.txt and SECURITY_REVIEW_1.7.3.md for the current candidate update. Do not install a flagged older updater.
...
- Records every internal pool return with a `PR-YYYYMMDD-####` reference, date,
  amount, reason, project head, allocation, and original withdrawal sources.
- Keeps pool returns separate from cash surrender, bank redeposit, expenses,
  payments, and withdrawals, so no transaction is double counted.
- Allows a pool return to be voided or restored while blocking unsafe undo when
  its cash was reallocated or reopening would create a third active PC account.
- Groups allocation editing, return-to-pool, surrender, and correction actions
  into one compact Allocation Actions menu.
- Removes misleading project names from Direct Procurement funding choices;
  the creation project remains visible only as audit context.

The update also retains all v1.6.9 features:

- Limits each registered project head to two active Petty Cash accounts across
  projects while excluding every Direct Procurement allocation from that count.
- Keeps the third active Petty Cash account blocked until one of the first two
  is completed, surrendered, redeposited, or voided.
- Automatically adds suppliers used in ordinary expenses, imported expense
  batches, edited expenses, and Direct Procurement allocations to Contacts.
- Backfills supplier contacts from existing qualifying expense and DP records
  without changing any financial transaction.
- Adds safer Contacts controls for adding, editing, deleting, viewing deleted
  entries, and restoring them while retaining historical records.
- Excludes payroll and employee cash-advance payees from automatic supplier
  creation and excludes deleted suppliers from new DP suggestions.

The update also retains all v1.6.8 features:

- Shows every non-void Petty Cash and Direct Procurement allocation directly
  in the Payments / Funding window, including its issued and remaining amounts.
- Allows a usable allocation to be selected directly for an expense payment;
  zero-balance allocations remain visible but cannot be used.
- Uses calculated allocation balance instead of stale saved status when deciding
  whether a funding source is available.
- Adds a dedicated **Batch Expense Drafts** entry point covering both manual and
  imported draft batches.
- Adds Edit Selected and double-click editing for staged batch-expense rows, with
  totals and funding validations recalculated before the row is saved.
- Keeps staged row removal and ensures edits to committed expenses return them
  to Unverified for rechecking.

The update also retains all v1.6.7 features:

- Adds **Reopen for Correction** to committed weekly payrolls.
- Reverses the linked expense payments and restores their PC, DP, or bank
  balances before returning attendance and deductions to weekly staging.
- Preserves the original payroll and expense as an immutable audit snapshot.
- Recommitting creates a new payroll and expense linked to the superseded
  original, preventing duplicate expenses and salary deductions.
- Prevents payroll-linked expenses from being voided or restored separately
  from their payroll workflow.

The update also retains all v1.6.6 features:

- Adds a compact Payments / Funding workspace to every expense.
- Supports several PC, DP, or bank payment sources for one expense or payroll.
- Safely reassigns a mistaken payment source and recalculates both allocations,
  expense paid/outstanding totals, and audit history.
- Allows an insufficient source to fund the available partial amount, leaving
  the balance visibly outstanding for the next source.
- Adds a payroll PDF choice between Summary Only and Full Ledger with Daily
  Attendance.

The update also retains all v1.6.5 features:

- Saves batch cash advances and batch expenses as persistent drafts in the
  existing SQLite database, without posting any financial transaction.
- Opens saved drafts from either batch window and keeps the same draft
  reference when saving further corrections.
- Returns to the prior batch window with every staged row retained when the
  final confirmation or authorization is cancelled.
- Adds Edit Selected and double-click editing for staged cash advances,
  including amount, purpose, repayment plan, weekly limit, and the shared
  batch effective date.
- Marks a used draft as committed for audit history after the financial batch
  succeeds.

- Adds searchable Cash Advance PDF exports with checkbox filters for Salary
  Deduction, Cash Repayment, Bank Repayment, and Manual / Mixed.
- Includes current advance balances, matching recovery history, totals, and
  pending salary deductions in a readable A4 landscape report.
